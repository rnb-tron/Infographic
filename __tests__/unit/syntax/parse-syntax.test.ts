...
`;
  ...
  it('ignores markdown code fences and comment-only lines', () => {
    const input = `
\`\`\`infographic
# generated by AI
// another comment
...
`;
    ...
    expect(result.errors).toHaveLength(0);
    expect(result.options.template).toBe('sales-dashboard');
    expect(result.options.themeConfig?.palette).toEqual(['#fff', '#000']);
    expect(result.options.data?.items?.[0]).toMatchObject({
      label: 'A',
      value: 1,
    });
...
`;
  ...
  it('accepts boolean values case-insensitively', () => {
    const input = `
design
  structure sequence-timeline
    showIcon True
...
`;
    ...
    expect(result.options.data?.relations?.[0]).toMatchObject({
      from: 'A',
      to: 'B',
      showArrow: 'false',
    });
